...
export default function RiderEarnings() {
    // 1. Calculate Real Stats from Mock LocalStorage
    ...
    return (
        <div className="p-6 space-y-8">
                    ...
                    {weeklyData.map((val, i) => (
                        <div key={i} className="flex-1 flex flex-col items-center gap-2 group cursor-pointer">
                            ...
                            >
                                ...
                                {i === weeklyData.length - 1 && <div className="absolute top-0 left-0 right-0 h-1 bg-white/50" />}
                            </div>
            ...
            <div className="bg-card rounded-2xl border border-border pb-4">
                ...
                {[1, 2, 3].map((_, i) => (
                    <div key={i} className="px-6 py-4 flex items-center justify-between hover:bg-accent/30 transition-colors cursor-pointer">
                        <div className="flex items-center gap-4">
                            <div className="w-10 h-10 rounded-full bg-emerald-500/10 flex items-center justify-center text-emerald-500">
                                <DollarSign className="w-5 h-5" />
                            </div>
                            <div>
                                <h4 className="font-bold text-foreground text-sm">Weekly Payout</h4>
