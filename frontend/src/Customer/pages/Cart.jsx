...
export default function Cart() {
  const { cart, removeFromCart, getCartTotal, addToCart } = useContext(CartContext);
  ...
  return (
    <div className="container min-h-[60vh] py-10">
      ...
      <div className="space-y-4">
        {cart.map((item) => (
          <div key={item.id} className="bg-white p-6 rounded-xl shadow-sm border border-gray-100 flex items-center justify-between transition-all hover:shadow-md">
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                <h4 className="text-lg font-bold text-[#2E2E2E]">{item.name}</h4>
                <p className="text-gray-500 text-sm">Rs {item.price} per unit</p>
              </div>
            </div>
            ...
            <div className="flex items-center gap-6">
              ...
              <div className="flex items-center gap-3 bg-gray-50 rounded-full px-2 py-1 border border-gray-200">
                ...
                <span className="font-bold w-6 text-center text-[#2E2E2E]">{item.quantity}</span>
      ...
      <div className="mt-12 flex flex-col items-end border-t pt-8">
        <div className="text-right mb-6">
          ...
          <span className="text-4xl font-bold text-[hsl(var(--primary))]">Rs {getCartTotal()}</span>
        </div>
        <button
