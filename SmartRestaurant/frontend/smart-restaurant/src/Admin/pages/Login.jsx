...
const Login = () => {
  ...
  const getTheme = (currentRole) => {
    switch (currentRole) {
      case "rider":
        return {
          pageBg:
            "bg-gradient-to-br from-teal-50 via-teal-100/30 to-emerald-50",
          ...
          inputClass:
            "text-slate-700 bg-white border-teal-100 focus:border-teal-500 focus:ring-teal-500/20",
      ...
      case "admin":
        return {
          ...
          inputClass:
            "text-gray-800 bg-white border-gray-200 focus:border-amber-500 focus:ring-amber-500/20",
      ...
      default: // customer
        return {
          ...
          inputClass:
            "text-slate-700 bg-white border-blue-100 focus:border-blue-500 focus:ring-blue-500/20",
  ...
  return (
    <div
      className={`min-h-screen flex items-center justify-center p-4 transition-colors duration-500 ${theme.pageBg} relative`}
    >
      {/* Dynamic Background Elements */}
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        <div
          className={`absolute -top-20 -left-20 w-96 h-96 rounded-full blur-3xl opacity-30 transition-colors duration-700 ${
            role === "rider"
              ? "bg-teal-300"
              : role === "admin"
              ? "bg-amber-100"
        ...
        />
        <div
          className={`absolute bottom-0 right-0 w-80 h-80 rounded-full blur-3xl opacity-30 transition-colors duration-700 ${
            role === "rider"
              ? "bg-emerald-200"
              : role === "admin"
              ? "bg-gray-200"
      ...
      <Card
        className={`w-full max-w-md relative z-10 transition-all duration-300 ${theme.cardClass}`}
      >
        <div className="text-center mb-8">
          <h1
            className={`text-3xl font-bold bg-clip-text text-transparent transition-all duration-300 ${theme.titleGradient}`}
          >
            OnTheGo
          </h1>
        ...
        <div className="flex p-1.5 bg-slate-100/80 rounded-xl mb-8 relative border border-slate-200/50">
          ...
          {roles.map((r) => (
            <button
              ...
              className={`flex-1 flex items-center justify-center gap-2 py-2.5 text-sm font-semibold rounded-lg relative z-10 transition-all duration-300 ${
                role === r.id ? theme.activeTabClass : theme.inactiveTabClass
              }`}
            >
              <r.icon size={16} />
              {r.label}
        ...
        <form onSubmit={handleLogin} className="space-y-6">
          <Input
            ...
            type="email"
            placeholder={
              role === "admin" ? "admin@onthego.com" : "you@example.com"
            }
            icon={Mail}
          ...
          <Button
            type="submit"
            variant="ghost"
            className={`mt-8 w-full py-3 text-white font-bold rounded-xl bg-gradient-to-r from-indigo-500 to-purple-600 hover:from-indigo-600 hover:to-purple-700 shadow-lg hover:shadow-xl transition-all duration-300 transform hover:-translate-y-0.5 ${theme.buttonClass}`}
          >
